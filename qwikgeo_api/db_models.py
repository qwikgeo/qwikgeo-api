...
class GroupUser(models.Model):
    ...
    id = fields.IntField(pk=True)
    group_id: fields.ForeignKeyNullableRelation[Group] = fields.ForeignKeyField(
...
class ItemReadAccessList(models.Model):
    ...
    portal_id: fields.ForeignKeyRelation[Item] = fields.ForeignKeyField(
        ...
        related_name="item_read_access_list",
        to_field="portal_id",
        on_delete='CASCADE'
    )
...
class ItemWriteAccessList(models.Model):
    ...
    portal_id: fields.ForeignKeyRelation[Item] = fields.ForeignKeyField(
        model_name="models.Item",
        related_name="item_write_access_list",
        to_field="portal_id",
        on_delete='CASCADE'
    )
    name = fields.CharField(500)
...
class Table(models.Model):
    ...
    table_id = fields.CharField(50, pk=True)
    created_time = fields.DatetimeField(auto_now_add=True)
    modified_time = fields.DatetimeField(auto_now=True)
...
class Layer(models.Model):
    ...
    layer_id = fields.CharField(max_length=1000)
    ...
    style = fields.JSONField(default={}, null=True)
    paint = fields.JSONField(default={}, null=True)
    layout = fields.JSONField(default={}, null=True)
    fill_paint = fields.JSONField(default={}, null=True)
    border_paint = fields.JSONField(default={}, null=True)
    bounding_box = fields.JSONField()
...
TableOut_Pydantic = pydantic_model_creator(Table, name="TableOut", exclude=("user","item.user","item.maps",))
Map_Pydantic = pydantic_model_creator(Map, name="Map")
MapOut_Pydantic = pydantic_model_creator(Map, name="MapOut", exclude=("user","item.user","item.tables",))
Item_Pydantic = pydantic_model_creator(Item, name="Item")
ItemOut_Pydantic = pydantic_model_creator(Item, name="ItemOut", exclude=("user","tables.user"))
